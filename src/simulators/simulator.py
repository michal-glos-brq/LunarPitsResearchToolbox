...
class RemoteSensingSimulator:
    ...
    def start_simulation(
        ...
        for instrument in self.instruments:
            ...
            try:
                height = np.linalg.norm(
                    instrument.project_boresight(self.current_simulation_timestamp_et).spacecraft_relative
                )
                instrument_state.heights.add(height)
            ...
            try:
                fov_width = instrument.recalculate_bounds_to_boresight_distance(self.current_simulation_timestamp_et)
                instrument_state.fov_widths.add(fov_width)
            except Exception as e:
        ...
        simulation_timekeeper = timedelta(seconds=0)
        simulation_duration_formatted = format_td(simulation_duration)
        pbar = (
            tqdm(total=total_seconds, unit="time_step", disable=SUPRESS_TQDM) if interactive_progress else nullcontext()
        )
            ...
            while self.current_simulation_timestamp < end_time:
                try:
                    ...
                    if max_speed_counter == 0:
                        ...
                        spacecraft_position, spacecraft_velocity = self.instruments[
                            0
                        ].calculate_spacecraft_position_and_velocity(
                            self.current_simulation_timestamp_et, self.kernel_manager.main_reference_frame
                        )
                    ...
                    for instrument in self.instruments:
                        ...
                        distance = rank - (
                            instrument_state.heights.maximum
                            + instrument_state.fov_widths.maximum
                            + self.filter.hard_radius
                        )
                        if distance > 0:
                        ...
                        try:
                            ...
                            projection: ProjectionPoint = instrument.project_boresight(
                                self.current_simulation_timestamp_et
                            )
                    ...
                    if len(distances_to_tresholds) > 0:
                        self._computation_timedelta = max(
                            SIMULATION_STEP, min(distances_to_tresholds) / max_speed.maximum
                        )
                    else:
                        self._computation_timedelta = SIMULATION_STEP
                    ...
                    if time.time() - real_time > SIM_STATE_DUMP_INTERVAL:
                        if interactive_progress:
                            ...
                            block_lines = [
                                state.dump_status_lines() for state in self.instrument_simulation_states.values()
                            ]
                            transposed = zip(*block_lines)
                            aligned_output = "\n".join(
                                "   ".join(f"{s:<16}" for s in line_parts) for line_parts in transposed
                            )
                            state_string = (
                            ...
                            if current_task is not None:
                                ...
                                current_task.update_state(
                                    ...
                                    meta={
                                        "state": state,
                                        "progress [%]": 100 * simulation_timekeeper / simulation_duration,
                                    },
                                )
                    ...
                    if interactive_progress:
                        ...
                        logging.info(
                            f"Simulated time: {format_td(simulation_timekeeper)} / {simulation_duration_formatted}"
                        )
